...
import { Menu, X, Home, User, Folder, Code, Mail, ArrowRight } from "lucide-react";
...
const menuItems = [
  ...
  { id: "tech", label: "Tecnologías", icon: <Code size={20} />, href: "#tech" },
...
export const Navbar = () => {
  ...
  return (
    <LayoutGroup>
      {/* --- DESKTOP NAVBAR (Glossmorphism Gris igual a Mobile) --- */}
      <nav className="
        hidden md:block fixed top-0 left-1/2 z-50 mt-6 -translate-x-1/2 
        /* FONTO ESTILO BURBUJA */
        bg-zinc-800/60 backdrop-blur-[20px] 
        border border-white/20 shadow-lg rounded-full 
        overflow-hidden
        /* PUNTOS DE LUZ IGUALES A MOBILE */
            ...
            return (
              <a key={item.id} href={item.href} className="relative group">
                ...
                <span className={`
                  ...
                  ${isActive ? "bg-[#FF6F00] text-white scale-110" : "text-zinc-400 hover:text-white"}
                `}>
                  {item.icon}
                </span>
              </a>
          ...
          {!isOpen ? (
            <motion.div
              ...
              className="
                ...
                shadow-[0_12px_40px_rgba(0,0,0,0.3)] cursor-pointer overflow-hidden 
                bg-zinc-800/60 backdrop-blur-[20px] 
            ...
            <React.Fragment key="open-panel">
              ...
              <motion.div
                layoutId="glass-panel"
                className="
                  text-white p-6 rounded-[2.5rem] shadow-[0_24px_64px_rgba(0,0,0,0.4)] 
                  w-[280px] bg-zinc-900/85 backdrop-blur-[30px] 
              ...
              >
                <div className="flex justify-between items-center mb-6 pb-4 border-b border-white/10 relative z-10">
                  <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-[#FF6F00]">
                    Navegación
                  </span>
                  ...
                  {menuItems.map((item, i) => (
                    <motion.a
                      ...
                      className={`
                        ...
                        ${activeSection === item.id ? "bg-white/10" : "bg-white/0 hover:bg-white/5"}
                      `}
                    >
                      <div className="flex items-center gap-4">
                        <span className={`
                          ${activeSection === item.id ? "text-[#FF6F00]" : "text-white/60"} 
                          group-hover:text-[#FF6F00] transition-colors
                        `}>
                          {item.icon}
                        </span>
